...
func main() {
	...
	category := flag.Int("category", 1440, "category option passed from cmd")
	...
	WHERE(
		id=360385 AND active = True AND integration_type IN (%v) AND show_for_client_id = 405506
		AND (last_run <= '%v' OR last_run IS NULL)
	) ORDER BY last_run ASC limit 1;`, *integrtion_type, sinceTime)
